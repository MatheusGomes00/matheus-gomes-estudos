...
public class Rei extends ChessPiece {
    ...
    public boolean[][] possibleMoves() {
        ...
        // movimento especial castling... trocar o rei pela torre na primeira joga de ambos
        if(getMoveCount() == 0 && !chessMatch.getCheck()){
            // para o lado direito do rei
            Position position1 = new Position(position.getRow(), position.getColumn() + 3);
            if(testRookCastling(position1)){
                Position p1 = new Position(position1.getRow(), position.getColumn() + 1);
                Position p2 = new Position(position1.getRow(), position.getColumn() + 2);
                if(getBoard().piece(p1) == null && getBoard().piece(p2) == null){
                    mat[position.getRow()][position.getColumn() + 2] = true;
                }
            }
            // para o lado esquerdo do rei
            Position position2 = new Position(position.getRow(), position.getColumn() - 4);
            if(testRookCastling(position2)){
                Position p1 = new Position(position1.getRow(), position.getColumn() - 1);
                Position p2 = new Position(position1.getRow(), position.getColumn() - 2);
                Position p3 = new Position(position1.getRow(), position.getColumn() - 3);
